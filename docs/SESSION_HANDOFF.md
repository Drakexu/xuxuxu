...
- Completed:
  - Added feed interaction layer (client-side persisted):
    ...
    - `�����ղ�` filter on both aggregate and single-character feed
...
- Completed:
  - Added source-trace loop for unlocked characters copied from square:
    ...
    - show `ԭ��ɫ����` CTA in both normal mode and manage mode
...
- Completed:
  - Expanded feed interaction filters:
    - added `����ϲ��` on both aggregate and single-character feeds
    - retained `�����ղ�` filter
  - Added `ϲ��` KPI card beside `�ղ�` for faster social-state scanning.
- Validation:
  - `npm run lint` -> pass
  - `npx tsc --noEmit` -> pass
  - `npm run build` -> pass
...
- Completed:
  - Added feed sort strategies driven by interaction signals:
    ...
    - `ϲ������`
    - `�ղ�����`
  - Sort now works alongside existing filters (`����ϲ��` / `�����ղ�` / event tabs / search).
- Validation:
  - `npm run lint` -> pass
  - `npx tsc --noEmit` -> pass
  - `npm run build` -> pass
...
- Square list page (`app/square/page.tsx`):
  ...
  - Added per-card popularity badge: `���� X �� ���� Y`
...
- Added square -> creator fork flow:
  - `app/square/page.tsx`: each card now has `��������` action
  - `app/square/[characterId]/page.tsx`: detail operations now include `��������`
- Added template prefill in creator page:
  ...
  - Defaults fork visibility to `private` and role name to `<ԭ��> �� ����`
  - Shows template-loaded hint and supports one-click `���ģ��`
- Validation:
  - `npm run lint` passed
  - `npm run build` passed
...
- Updated workbench lineage display in `app/characters/page.tsx`:
  ...
  - creator cards now show `�����Թ㳡` and expose `ģ������` jump back to square detail
...
- Completed:
  ...
  - Added dual-guard loop (backend + UI):
    ...
    - chat UI surfaces guard warning and applies a final local strip for accidental `User:/�û�:/��:` speaker lines.
...
- Completed:
  ...
  - Improved chat stage status presentation:
    ...
    - return-to-bottom button now includes unread count directly (`�ص��ײ� �� N`), reducing duplicated hint noise.
- Validation:
  - `npm run -s lint` passed
  - `npx tsc --noEmit` passed
  - `npm run -s build` passed
...
- Files changed:
  ...
  - `app/globals.css`
- Completed:
  - Added a dedicated "��������" section on `/home` with 3 actionable entry cards:
    - ����Ȧ (`MOMENT_POST`)
    - �ռ� (`DIARY_DAILY`)
    - �ճ�Ƭ�� (`SCHEDULE_TICK`)
  - Each card now shows:
...
- Completed:
  ...
  - Added navigation/product loop entry points:
    - main sidebar + mobile dock includes `Ǯ��`
    - square list/detail operations include `Ǯ������` jump
- Validation:
  - `npm run -s lint` -> pass
  - `npm run -s build` -> pass
  - `npx tsc --noEmit` -> pass
...
- Completed:
  ...
  - Updated square detail UI:
    - displays comment author identity (`��/������/�û�xxxx`)
...
- Completed:
  ...
  - Added one-click manual schedule generation action:
    - `��һ���ճ�Ƭ��` calls `/api/chat` with `SCHEDULE_TICK` for current conversation
...
- Completed:
  - Home page upgrades:
    - life-hub cards now include cadence health (`�������� / �ӳ� / �ȴ�����`) based on expected hourly/daily rhythm.
    - added one-click `��һ���ճ�Ƭ��` action for current focus role (uses `SCHEDULE_TICK`, then reloads feed).
    - added operation-role hint (`������ɫ`) to make manual actions explicit.
  - Square page upgrades:
    - added queue preview block in ������� showing unlocked roles with active state, price, and hot score.
    - added direct quick actions for queue items (`����` / `��̬`).
    - added quick filters for `�����ҵĶ���` and `�����Ѽ���`.
- Validation:
  - npm run -s lint -> pass
  - npx tsc --noEmit -> pass
  - npm run -s build -> pass
