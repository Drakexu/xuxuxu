...
export default function DraQTDashboard() {
  ...
  const loadDaily = useCallback(async () => {
    await Promise.all([
      fetchJson<Account>("account.json").then((d) => setAccount(d && d.nav != null ? d : null)),
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: "Cash", value: `$${fmtShort(account.cash)}`, icon: Wallet, detail: `${fmt(account.nav ? (account.cash / account.nav * 100) : 0, 1)}% of NAV` },
                  ].map((card, i) => (
                    <motion.div
                      key={i}
